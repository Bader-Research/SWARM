...
static char *inputLabel(const int input) {
  char *a;
  switch(input) {
...
/****************************************************/
static void swarm_countsort(int q,
		     s_struct *lKey,
		     s_struct *lSorted,
		     int *pdf,
		     int R,
...
static void print_graph(bool inorder, bool sequential, const int *S, const int num_pts)
{
    int i, current;
...
static void print_full_graph(const int *S, const int *P, const int *C, const int num_pts)
{
    int cur_vertex;
    int i;
...
static int count_colors(const int *colors, const int num_pts)
{
    int total_used;
    int *ref_cnt;
    int i;
...
/* points from 0 to n. */
static void permute_array(int *S, const int num_pts)
{
    int		i, rnum, itmp;
    double  dtmp;
...
static int *generate_array(const int num_pts, const int input, THREADED)
{
    int *result;
    int i;
    
    result = (int *) SWARM_malloc(sizeof(int) * num_pts , TH);

    switch(input) {
    case _INP_RANDOM : {
      int	*list;
      list = (int *) SWARM_malloc(sizeof(int) * num_pts , TH);
...
static void S_to_P(const int *S, int *P, const int num_pts, THREADED)
{
    int i;
...
static int lsb_diff(const int in_1, const int in_2)
{
...
static bool verify_colormap(const int *S, const int *P, const int *C, const int num_pts)
{
    int current = 0;
    int i;
...
static void serial_three_color(const int *S, const int num_pts, int *init_colors)
{
#if 0
...
static void three_color(const int *S, int *second_colors, const int num_pts, THREADED)
{
  ...
  int *init_colors = NULL;
    ...
    init_colors = (int *) SWARM_malloc(sizeof(int) * num_pts, TH);
    second_colors = (int *) SWARM_malloc(sizeof(int) * num_pts, TH);
    
    /* Predecessor array */
    P = (int *) SWARM_malloc(sizeof(int) * num_pts, TH);

    /* Sort structs - 2-ints, vertex & color */
    bag_in = (s_struct *) SWARM_malloc(sizeof(s_struct) * num_pts, TH);
    bag_out = (s_struct *) SWARM_malloc(sizeof(s_struct) * num_pts, TH);

    /* 256 ints, holding PDF of sorted color array */
    pdf = (int *) SWARM_malloc(sizeof(int) * NUM_HBINS, TH);
    ...
    */
    ...
    swarm_countsort(num_pts, bag_in, bag_out, pdf,
...
*/
static void run_the_code(const int num_pts, const int num_iterations, 
		  double *min_time,
		  double *max_time,
		  double *avg_time,
		  double *sigma,
...
static void symbreak(const int input, int cur_size, int num_iterations, int testrun, THREADED)
{
...
static void SymBreak_main(THREADED) {
